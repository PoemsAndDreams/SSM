 ...
 * 若类和方法都有@RequestMapping，此时控制器方法所匹配的请求的请求路径为/test/hello
 ...
 * @GetMapping,@PostMapping,@DeleteMapping,@PutMapping(匹配各自的类型请求）
 * 4、@RequestMapping注解的params属性
 * 作用：通过请求的请求参数匹配请求，即浏览器发送的请求的请求参数必须满足params属性的设置
 * params可以使用四种表达式：
 * "param"：表示当前所匹配请求的请求参数中必须携带param参数。如：请求中携带username : @{/hello?username=admin} 或 @{/hello(username='admin')}
